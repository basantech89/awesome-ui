...
export default function spacing(spacingOptions: SpacingOptions = {}): Spacing {
  const { unit, spacerPerSpacingUnit } = deepMerge(
    defaultSpacingOptions,
    spacingOptions
  )

  const computeSpacing = (spacer: SpacingUnit) =>
    `${spacer * spacerPerSpacingUnit}${unit}`
  ...
  const getSpacing = (spacer: SpacingUnit) =>
    spacer in spacers
      ? precomputedSpacing[spacer as Spacer]
      : computeSpacing(spacer)
